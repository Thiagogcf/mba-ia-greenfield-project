...
export class CreateVideos1786402715912 implements MigrationInterface {
    name = 'CreateVideos1786402715912'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TYPE "public"."videos_status_enum" AS ENUM('draft', 'processing', 'ready', 'failed')`);
        await queryRunner.query(`CREATE TABLE "videos" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "channel_id" uuid NOT NULL, "title" character varying(255) NOT NULL, "description" text, "status" "public"."videos_status_enum" NOT NULL DEFAULT 'draft', "public_id" character varying(11) NOT NULL, "file_name" character varying(255) NOT NULL, "content_type" character varying(100) NOT NULL, "file_size" bigint NOT NULL, "original_key" character varying(512) NOT NULL, "thumbnail_key" character varying(512), "duration_seconds" integer, "metadata" jsonb, "failure_reason" text, "s3_upload_id" character varying(255), "created_at" TIMESTAMP NOT NULL DEFAULT now(), "updated_at" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "UQ_39a1f0fe7991162aace659078ec" UNIQUE ("public_id"), CONSTRAINT "PK_e4c86c0cf95aff16e9fb8220f6b" PRIMARY KEY ("id"))`);
